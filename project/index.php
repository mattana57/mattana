...
.navbar { background: rgba(26, 0, 40, 0.85); backdrop-filter: blur(10px); position: sticky; top: 0; z-index: 1000; border-bottom: 1px solid rgba(187, 134, 252, 0.2); }

/* --- การ์ดสินค้าแบบใหม่ (กดได้ทั้งใบ) --- */
.product-card {
    background: rgba(255, 255, 255, 0.03); border: 1px solid rgba(255, 255, 255, 0.1);
    backdrop-filter: blur(15px); border-radius: 15px; transition: all 0.3s ease;
    cursor: pointer; position: relative; overflow: hidden;
}
.product-card:hover {
    transform: translateY(-10px); background: rgba(255, 255, 255, 0.08);
    border-color: #bb86fc; box-shadow: 0 10px 30px rgba(0,0,0,0.5), 0 0 15px rgba(187, 134, 252, 0.3);
}

/* --- ปุ่มสีใหม่ (Neon Mystery) --- */
.btn-neon-purple {
...
.section-title { border-left: 5px solid #f107a3; padding-left: 15px; margin-bottom: 25px; font-weight: 600; text-shadow: 0 0 10px rgba(241, 7, 163, 0.3); }
.badge-cart { position: absolute; top: -5px; right: -5px; background: #f107a3; color: white; font-size: 10px; padding: 3px 7px; border-radius: 50%; }
.modern-btn { background: rgba(255,255,255,0.1); color:#fff; border: 1px solid rgba(255,255,255,0.2); padding:8px 20px; border-radius:30px; text-decoration:none; transition:.3s; }
.modern-btn:hover, .active-category { background: #bb86fc; color:#120018; border-color: #bb86fc; }
</style>
</head>
<body>
...
<div class="container mt-4">
    <div id="mainBanner" class="carousel slide carousel-fade shadow-lg rounded-4 overflow-hidden" data-bs-ride="carousel">
        <div class="carousel-inner">
            <div class="carousel-item active"><img src="images/BN1.png" class="d-block w-100" style="height:400px; object-fit:cover;"></div>
            <div class="carousel-item"><img src="images/BN2.png" class="d-block w-100" style="height:400px; object-fit:cover;"></div>
        </div>
    </div>
</div>
        ...
        <div class="row mb-5">
            ...
            <div class="col-md-3 mb-4">
                <div class="card product-card p-3 h-100" onclick="location.href='product.php?id=<?= $p['id'] ?>'">
                    <img src="images/<?= $p['image']; ?>" class="img-fluid mb-2 rounded" style="height:180px; object-fit:cover;">
                    ...
                    <div class="mt-auto d-flex gap-2" onclick="event.stopPropagation();">
                        <button onclick="addToCart(<?= $p['id'] ?>)" class="btn btn-neon-purple btn-sm w-50">ลงตะกร้า</button>
                        <a href="add_to_cart.php?id=<?= $p['id'] ?>&action=buy" class="btn btn-neon-pink btn-sm w-50 d-flex align-items-center justify-content-center text-decoration-none">สั่งซื้อ</a>
                    </div>
                </div>
            </div>
            <?php } ?>
